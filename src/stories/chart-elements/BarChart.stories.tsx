...
export const CustomTooltipSimple: Story = {
  args: {
    ...
    customTooltip: (props: CustomTooltipType) => {
      ...
      return (
        <div className="w-56 rounded-garmz-default text-garmz-default bg-garmz-background p-2 shadow-garmz-dropdown border border-garmz-border">
          <div className="flex flex-1 space-x-2.5">
            <div className={`w-1.5 flex flex-col bg-${categoryPayload.color}-500 rounded`} />
            <div className="w-full">
              <p className="font-medium text-garmz-content-emphasis">{label}</p>
              <div className="flex items-center justify-between space-x-8">
                <p className="text-right text-garmz-content whitespace-nowrap">
                  {categoryPayload.dataKey}
                </p>
                <p className="font-medium text-right whitespace-nowrap text-garmz-content-emphasis">
...
export const CustomTooltipPreviousDay: Story = {
  args: {
    ...
    customTooltip: (props: CustomTooltipType) => {
      ...
      return (
        <div className="w-56 translate-y-14 flex items-center justify-between rounded-garmz-default text-garmz-default bg-garmz-background p-2 shadow-garmz-dropdown border border-garmz-border">
          <span className="text-right text-garmz-content whitespace-nowrap">{dataKey}</span>
          <div className="flex items-center space-x-2">
            <span className="font-medium text-right whitespace-nowrap text-garmz-content-emphasis">
...
export const CustomTooltipComplex: Story = {
  args: {
    ...
    customTooltip: (props: CustomTooltipType) => {
      ...
      return (
        <div className="rounded-garmz-default bg-garmz-background p-2 shadow-garmz-dropdown border border-garmz-border">
          <div className="flex flex-1 space-x-2.5">
            <div className={`w-1 flex flex-col bg-${categoryPayload.color}-500 rounded`} />
            <div className="w-full">
              <p className="text-garmz-default font-medium text-garmz-content-emphasis">
                {dataKey}
              </p>
              <p className="text-garmz-default text-garmz-content-subtle">{label}</p>
              <p className="mt-2 font-medium whitespace-nowrap text-garmz-content-emphasis">
                {currencyValueFormatter(value)}
              </p>
              {percentage ? (
                <div className="mt-1 flex items-end space-x-2">
                  <div
                    className={`inline-flex text-garmz-default px-1.5 py-0.5 bg-${badgeColor}-100 text-${badgeColor}-600 rounded`}
                  >
                    {percentage > 0 ? "+" : null}
                    {percentage.toFixed(1)}%
                  </div>
                  <div className="whitespace-nowrap text-garmz-default text-garmz-content-subtle">
                    from previous month
                  </div>
                </div>
              ) : null}
